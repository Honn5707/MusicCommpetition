...
//        recaptchaUtilities.certify(recaptchaToken);
        ...
        if(request.provider() == Provider.LOCAL) {
            newMember = Member.builder().provider(provider).providerId(providerId).nickname(nickname)
                    .password(passwordEncoder.encode(request.password())).email(request.email()).build();
        } else {
            newMember = Member.builder().provider(provider).providerId(providerId).nickname(nickname)
                    .password(null).email(request.email()).build();
        }
